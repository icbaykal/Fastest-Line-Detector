...
void link_open(int option, Liste *StringListID)
/*
					...
					if (option == 1) {
						...
						if (weight > thresh_sig) {
							if (closed_only == FALSE) {
								no_lists_written++;
								//if (flag == TRUE)fprintf(fp_out, "  -1    0\n");
								flag = TRUE;
								...
								for (loop3 = 1;loop3 <= loopindex;loop3++)
								{
									...
									ORTPoint *StringList = (ORTPoint*)malloc(sizeof(ORTPoint));
									StringList->Col = (double)xpix[loop3] - ((double)(width)) / 2.0;
									StringList->Row = (double)ypix[loop3] - ((double)(height)) / 2.0;
									//Now add this element to the list
									AddElmList(StringListID[list_no], (long)StringList);
								}								
							}
						}
					}
...
void link_closed(int option, Liste *StringListID)
/*
   option = 0  write list to disk
   option = 1  write significant lines to disk
*/
...
int link_and_save(int option, Liste *StringListID)
...
*/
{
	...
	//fprintf(fp_out, "%d %d\n", width, height);
	int NumberofStringListID = 0;
	/* link open edges */
	list_no = 0;
	link_open(option, StringListID);

	/* remove isolated points - of no interest to us here */
	remove_isolated();

	/* link closed edges */
	link_closed(option, StringListID);
...
void convert_chainpix(BIMG &img)
{
	//from main()
	set_aspect = FALSE;
	set_file_out = TRUE;
	...
	for (int i = 0; i < height; i++) {
		...
		{
			image[i][j] = img.buffer[(i*img.width + j) * 4];
		}
	}
	//fp_out = fopen("myfile.txt", "w");
	//if (fp_out == NULL) msg.Post("Error opening file");
}